SELECT username, default_tablespace, profile, authentication_type FROM
...
IDENTIFIED BY 
    root;
-- Grant Control
GRANT create session TO c##tech;
GRANT create table TO  c##tech;
GRANT create view TO c##tech;
GRANT create any trigger TO c##tech;
GRANT create any procedure TO c##tech;
GRANT create sequence TO c##tech;
GRANT create synonym TO c##tech;
GRANT ALL PRIVILEGES TO c##tech;

GRANT CONNECT TO c##tech;
GRANT RESOURCE TO c##tech;
GRANT DBA TO c##tech;
...
ALTER TABLE DEPT MODIFY DNAME VARCHAR2(14) NOT NULL UNIQUE;
...
SAL 		NUMBER(7,2),
           COMM 		NUMBER(7,2),
           DEPTNO 	NUMBER(2) CONSTRAINT FK_EMP_DEPTNO REFERENCES DEPT,  BRANCHNO 	INT CONSTRAINT FK_EMP_BRANCHNO REFERENCES BRANCH );  
...
-- Sort the all employees details on empno
SELECT 
    empno,ename,job,mgr,hiredate,sal,comm,deptno,branchno  
FROM emp
ORDER BY empno;
-- Sort the all employees details on department wise
SELECT  empno,ename,job,mgr,hiredate,sal,comm,deptno,branchno  
FROM emp
ORDER BY deptno;
-- Sort the all employees details as per department by emp no
SELECT deptno,empno,ename,job,mgr,hiredate,sal,comm,branchno    
FROM emp
